...
void uart_transmit(void * data, char length) {
  ...
  for(int i = 0; i < length; i++) {
    while(!PIR3bits.TX1IF) {}    // wait for the TXIF to be cleared, indicating the TXREG is empty, before writing
    TX1REG = ch[i];	            // load 8-bit data into the TXREG regsiter to start the transmission
  }
}
